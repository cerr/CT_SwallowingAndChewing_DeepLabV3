...
# Date: Oct 25, 2019
# Description: Script to fuse segmentations of pharyngeal constrictor muscle from three different views.
# Architecture :  Deeplab v3+ with resnet backbone
# Usage: python fuse_seg_cm.py [argv]
...
from . import run_seg_cm_ax
from . import run_seg_cm_cor
from . import run_seg_cm_sag
...
def main(argv):
    ...
    t0 = process_time()
    probMapAx, fName = run_seg_cm_ax.main(inputH5PathAx, batchSize)
    print(process_time() - t0)

    t1 = process_time()
    probMapSag = run_seg_cm_sag.main(inputH5PathSag, batchSize)
    print(process_time() - t1)

    t2 = process_time()
    probMapCor = run_seg_cm_cor.main(inputH5PathCor, batchSize)
